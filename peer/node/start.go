...
func startCmd() *cobra.Command {
	...
	//peer node start --peer-chaincodedev 是否是开发模式
	flags.BoolVarP(&chaincodeDevMode, "peer-chaincodedev", "", false,
		"Whether peer in chaincode development mode")

	//peer node start --o 配置orderer
...
func serve(args []string) error {
	// 当前peer只能在标准MSP下起作用
	// 因为在某些情形下MSP必须确保一个凭证中只能有一个身份
	// Idemix 类型不能够支持 *YET* 但是它能够通过简单的修复来支持它
	// 所以现在, 我们仅仅能够确保peer在标准MSP下来启动
	// 疑问:这几种类型有什么区别
